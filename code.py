...
def GibbsSampler(nchain, initialisation, data, param=param_defaut) :
   ...
   for i in range(nchain):
    ...
    ## Mise a jour de  Tau
    #scale = 1/beta
    chain[i+1,2] = rd.gamma(shape = params[2] + n/2, scale = 2/np.power(data[:, 1].sum() - chain[i,0] + chain[i,1]*np.power(chain[i,3], data[:, 0])[0], 2))
...
n = 27
data = np.array([[1, 1.5, 1.5, 1.5, 2.5, 4, 5, 5, 7, 8, 8.5, 9, 9.5, 9.5, 10, 12, 12, 13, 13, 14.5, 15.5, 15.5, 16.5, 17, 22.5, 29, 31.5],
